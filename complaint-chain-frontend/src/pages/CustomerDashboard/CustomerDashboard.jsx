...
const CustomerDashboard = () => {
  ...
  const handleSubmitComplaint = (e) => {
    ...
    const payload = { ...complaintData, submittedAt: new Date(complaintData.submittedAt).toISOString().slice(0,19) };

    axios.post("http://localhost:8081/Complaint-chain/api/complaints/submit", payload)
      .then(res => {
        ...
        {activeTab==="track" && (
          <div className="cust-tab-box">
            ...
            <table className="cust-table">
              ...
              <tbody>
                {complaints.map(c=>(
                  <tr key={c.id} onClick={()=>handleRowClick(c)} style={{backgroundColor: c.status==="In Progress"?"#fff3cd":""}}>
                    <td>{c.id}</td>
                    <td>{c.department}</td>
                    <td>{c.urgency}</td>
                    <td>{c.description}</td>
